...
# --- 1. تصميم الواجهة السيبرانية الفاخرة (Cyber-Split UI) ---
...
st.markdown("""
    ...
    .path-col { background: rgba(15, 15, 15, 0.9); border: 2px solid #bc13fe; border-radius: 15px; padding: 20px; min-height: 650px; box-shadow: 0 0 15px rgba(188, 19, 254, 0.2); }
...
TASI_SECTORS_2026 = {
    ...
    "الخدمات الاستهلاكية": ["4050.SR", "4190.SR", "4290.SR"],
    "الإعلام والترفيه": ["4070.SR", "4270.SR"],
    "تجزئة السلع الاستهلاكية": ["4006.SR", "4161.SR", "4163.SR"],
...
def perform_tcr_full_audit(symbol, op_log):
    try:
        ...
        buffett = [
            {"label": "سجل التوزيع", "val": f"{div_years}Y", "target": "> 10Y", "pass": div_years >= 10, "desc": "الاستمرارية تثبت صلابة الأرباح تاريخياً"},
            {"label": "مكرر P/E", "val": f"{pe:.1f}", "target": "< 18", "pass": pe <= 18, "desc": "شراء السعر العادل لضمان هامش الأمان"},
            {"label": "عائد التوزيع", "val": f"{div_yield:.1f}%", "target": "> 4%", "pass": div_yield >= 4, "desc": "العائد النقدي الفعلي الذي يستلمه المستثمر"},
            ...
            {"label": "نسبة السيولة", "val": f"{current_ratio:.2f}", "target": "> 1.2", "pass": current_ratio >= 1.2, "desc": "القدرة على سداد الالتزامات القصيرة"},
        ...
        lynch = [
            ...
            {"label": "جودة FCF", "val": f"{fcf/net_inc:.2f}", "target": "> 1.0", "pass": (fcf/net_inc) >= 1.0, "desc": "الأرباح هي كاش حقيقي وليست محاسبة"}
        ]
...
col_ctrl, col_lab = st.columns([1, 2]) # التحكم في عرض الأعمدة

with col_ctrl:
    st.markdown("### 🛰️ مركز القيادة")
    sector_key = st.selectbox("اختر القطاع:", list(TASI_SECTORS_2026.keys()))
    c1, c2 = st.columns(2)
...
with col_lab:
    st.markdown("### 🔬 مختبر الفحص المنقسم (15 مؤشر)")
    p1, p2 = st.columns(2)
    b_bench = p1.empty()
    l_bench = p2.empty()

if start:
    for sym in TASI_SECTORS_2026[sector_key]:
        res = perform_tcr_full_audit(sym, op_console)
        if res:
